...
# MAIN
if($help) {
  die "USAGE: $0 \n--use-aws|--use-virtualbox|--use-openstack|--use-vcloud \n[--working-dir <working dir path, default is 'target'> NOTE: this is only used if you are not using the config file!]\n[--config-file <config json file, default is 'vagrant_cluster_launch.json'> NOTE: this is only used if you are not using the new config files!]\n[--vb-ram <the RAM (in MB) to use with VirtualBox only, HelloWorld expects at least 9G, default is 12G>]\n[--vb-cores <the number of cores to use with Virtual box only, default is 2>]\n[--aws-ebs <EBS vol size in MB, space delimited>]\n[--use-default-config  including this flag means you are using the .cfg file to set configurations! So, don't forget to include this flag if you are using a .cfg file]\n[--launch-cluster <cluster-name of the cluster you want to launch> NOTE: this is used if you are using the config files instead of the json template]\n[--use-rsync this flag is used if you want to use the rsync line]\n[--vb-avoid-port-forwarding this flag is needed only if you are using virtualbox and want virtualbox to avoid taking over of host ports]\n[--avoid-master this flag makes sure that no jobs are scheduled on the master node]\n[--skip-launch]\n[--help]\n";
}
